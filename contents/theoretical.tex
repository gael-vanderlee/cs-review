...
Symptom measures in this literature must also be distinguished from related perceptual phenomena.
The dominant instrument is the Simulator Sickness Questionnaire (SSQ), which yields nausea, oculomotor, and disorientation subscales \cite{kennedySimulatorSicknessQuestionnaire1993}; it is frequently complemented by the single-item Fast Motion Sickness Scale (FMS) for continuous, in-task ratings \cite{keshavarzValidatingEfficientMethod2011}.
...
Distinct from these symptom measures, vection denotes the visually induced illusion of self-motion: it frequently covaries with sickness and figures in several etiological theories, but is not itself a symptom category \cite{hettingerVectionSimulatorSickness1990, keshavarzVectionVisuallyInduced2015}.
...
The most influential of these is \textit{sensory conflict theory}, which attributes sickness to inconsistency between visual, vestibular, and proprioceptive signals, or between current sensory input and the prior exposure history encoded by the observer \cite{reasonMotionSickness1975, reasonMotionSicknessAdaptation1978, omanMotionSicknessSynthesis1990}.
...
A related strand of recent VR work returns to the role of vection itself, showing that sickness intensifies when visually induced self-motion violates the user's expected motion or control state \cite{teixeiraUnexpectedVectionExacerbates2022, teixeiraInvestigatingRelativeContributions2024}; vection is treated here as a perceptual condition that may contribute to sickness rather than as a separate theory of motion sickness in its own right.
...
Finally, predictive-coding formulations recast sensory mismatch as a persistent prediction error between expected and observed input, offering a unifying computational vocabulary for the preceding theories \cite{nurnbergerMismatchVisualVestibularInformation2021}.

\subsection{Neural Recording and Modulation Modalities}
Cybersickness has historically been assessed through subjective reports, but a growing body of work uses neural recordings, and more recently non-invasive brain stimulation, to characterise the user's state during VR exposure or to actively modulate it.
...
These strengths are offset by limited spatial specificity, particularly for deep vestibular and autonomic structures, and by methodological sensitivities, such as motion artifacts, electrode density, reference choice, and baseline selection, which constrain the interpretation of reported effects.
...
Because it relies on hemodynamic rather than electrical signals, its temporal resolution remains substantially lower than that of EEG, and its optical penetration restricts measurement to superficial cortical regions.
