...
A residual nine studies have primary objectives in cognitive effects (4), recovery (1), dataset contributions (1), or other framings (3); their corpus-relevant findings are integrated into the relevant subsections, and their full discussion is deferred to \autoref{sec:discussion}.
...
Concurrent physiological recording is reported in 36 of 92 studies (39.1\%), with the criterion restricted to physiological sensors (electrocardiography, galvanic skin response, electro-oculography, electromyography, electrogastrography, photoplethysmography, respiration, skin temperature, blood pressure).
The most common adjuncts are electro-oculography (18 studies), electrocardiography (16), galvanic skin response (9), photoplethysmography (7), and electrogastrography and skin temperature (4 each).
Other co-recorded signals (head tracking, joystick, button input) are listed separately in the all-studies table.
...
The use of multiple measurement methods on the same exposure indicates that the operational definition of cybersickness varies across the corpus, which constrains cross-study comparability.
...
EEG signal preprocessing is also heterogeneous in filter selection.
Band-pass filtering ranges from very low frequencies (0.01 Hz) to high gamma (100 Hz), with the most frequent configurations being 1--50 Hz (9 studies), 0.5--50 Hz (7), 0.5--100 Hz (4), and 0.5--45 Hz (4); 25 studies do not specify a band-pass filter.
A 50 or 60 Hz notch filter is applied in 18 studies; 72 studies do not specify a notch filter.
The full distribution of band-pass configurations is shown in \autoref{fig:bandpass_distribution}.
...
The test-retest reliability of these spectral markers has been assessed in one study, which reported good reliability for delta, theta, and alpha power changes in frontal and central areas across multiple sessions \cite{limTestretestReliabilityVirtual2021}.
Delta-band normalisation has also been used as an objective marker of the post-exposure recovery time course \cite{wooRecoveryTimeVR2023}; the recovery literature is discussed further in \autoref{sec:discussion}.
...
Beyond band-power, a subset of studies has examined complex spectral and information-theoretic features of the EEG signal, including spectral entropy and skew \cite{fengComprehensiveExplorationMotion2025}, Gravity Frequency and Kolmogorov Complexity \cite{liuPilotStudyEEGBased2020, liuMeasuringVisuallyInduced2017}, amplitude-modulation features \cite{rosannePreprocessNotPreprocess2025}, and the mean frequency of the theta band as a direct sickness index \cite{liEEGbasedEvaluationMotion2022}.
Functional connectivity has also been investigated, although reporting is sparse: of the 16 studies that report a connectivity change, 7 report a mixed pattern, 4 a decrease, 3 an increase, and 2 no change.
Specific findings include a decrease in information transfer measured via transfer entropy with the onset of VIMS \cite{nurnbergerMismatchVisualVestibularInformation2021}, microstate-level alterations of canonical brain networks and decreased connectivity between visual cortex (V5) and vestibular processing regions \cite{namElectroencephalogramMicrostatesFunctional2022}, and phase-locking-value features that serve as inputs to deep classification models \cite{shenClassificationVisuallyInduced2024, huaDualpathwayEEGModel2025}.
The causal relevance of these spectral and connectivity patterns is suggested by intervention studies in which symptom mitigation correlates with a normalisation of the same EEG bands \cite{molefiVibromotorReprocessingTherapy2022, yeoEEGbasedAnalysisVarious2022}.
...
The direction of the hemodynamic effect is not uniform across the small fNIRS literature: regional perfusion increases in some cortical areas coexist with decreases in others, and fNIRS-derived hemoglobin signals have been used as input features for regression models predicting cybersickness severity \cite{renAssessingPassengersMotion2023}.
...
A subset of these studies uses multimodal pipelines that combine EEG with physiological signals or with VR content features.

Traditional machine-learning pipelines rely on hand-crafted features, with Support Vector Machines, k-Nearest Neighbours, and Linear Discriminant Analysis as the most common classifiers \cite{yuEEGbasedClassificationSystem2010, chaiEEGStudyVirtual2023}.
Features derived from EEG include Power Spectral Density, wavelet-derived energy ratios \cite{liVRMotionSickness2020}, spectral entropy and skew \cite{fengComprehensiveExplorationMotion2025}, and time-domain statistics such as signal variance \cite{mawalidClassificationEEGSignal2018}.
Dimensionality reduction is addressed through feature selection algorithms such as Correlation Feature Selection \cite{khoirunnisaaChannelSelectionEEGBased2018} and genetic algorithms \cite{weiGeneticFeatureSelection2011}.

Deep-learning pipelines learn task-relevant features directly from the data.
Convolutional Neural Networks (CNNs) are widely used to extract spatial and spectral features from EEG time series, spectrograms, or topographic scalp maps \cite{jeongCybersicknessAnalysisEEG2019}.
Temporal dynamics are captured through recurrent layers, including Long Short-Term Memory \cite{liaoUsingEEGDeep2020} and Gated Recurrent Units \cite{luoFeatureExtractionMethod2024}.
Other architectures include spiking neural networks \cite{yangPredictionDetectionVirtual2023} and shallow CNNs designed for real-time detection in mitigation systems \cite{uyanCDMSRealtimeSystem2024}.
Recent models employ hybrid architectures combining CNNs with LSTMs \cite{shenClassificationVisuallyInduced2024}, dual-pathway designs that process raw EEG and functional-connectivity inputs in parallel \cite{huaDualpathwayEEGModel2025}, and attention mechanisms that weight the contribution of different channels or time points \cite{huaEEGClassificationModel2024}.
Inputs to these models have diversified to include functional-connectivity matrices \cite{shenClassificationVisuallyInduced2024} and cross-frequency coupling asymmetries \cite{huaDetectionVirtualReality2024}.

Multimodal fusion combines EEG with peripheral physiological signals such as galvanic skin response and blood volume pulse \cite{sameriPhysiologydrivenCybersicknessDetection2024}, or with non-neural signals such as electromyography from postural muscles, which has been reported as more informative than concurrently recorded EEG in one study \cite{recentiPredictingMotionSickness2021}.
Other approaches use fNIRS as a regression input \cite{renAssessingPassengersMotion2023} or transfer knowledge from EEG-trained models to enable prediction from VR video content alone \cite{kimDeepCybersicknessPredictor2019}.
Explainable-AI techniques have been incorporated in a subset of studies to interpret model decisions \cite{sameriPhysiologydrivenCybersicknessDetection2024}.
...
Of the 38 classification studies, 9 (23.7\%) report accuracy as the sole discrimination metric, 10 (26.3\%) report only regression metrics ($R^2$, RMSE), 11 (28.9\%) report accuracy alongside both sensitivity/specificity and AUC or F1, 6 (15.8\%) report accuracy in combination with other metric types (sens/spec or AUC/F1 or regression), and 2 (5.3\%) report no quantitative performance.
This is an observational corpus-level fact; the interpretive critique --- that accuracy is uninformative under class imbalance and arbitrary thresholds --- is taken up in \autoref{sec:discussion}.
...
Addressing the threshold-heterogeneity problem raised above, one recent study develops a method for continuous, calibration-free detection directly from physiological data with dynamic, real-time ground-truth labels \cite{demirelSubjectivityContinuousCybersickness2025}.
...
Susceptible individuals show excessive parietal N2 ERP amplitudes and impaired theta-band phase synchronisation \cite{weiMotionSicknesssusceptibleParticipants2019}, higher P3 amplitudes during sensory-mismatch conditions \cite{ahnTemporalDynamicsVisually2020}, and differential spectral and source-space activation patterns during vection \cite{zhouBrainActivityDifferences2023}.
...
Two further studies investigated neurofeedback protocols in which users were trained to volitionally modulate their own brain activity.
...
Meta-analytic comparison across the 92 corpus studies reveals that VR display hardware substantially modulates EEG band-power responses, that stimulus-exposure duration is confounded with hardware, and that the corpus shows signs of funnel-plot asymmetry consistent with publication bias.
...
Across all reported classification accuracies in the corpus, Egger's regression test of funnel-plot asymmetry yields $p = 0.011$, consistent with possible publication bias toward high-accuracy results.
Egger's test is sensitive to between-study heterogeneity and small-study effects, and the present result should not be read as a definitive demonstration of bias; the interpretive caveats and the methodological recommendations that follow from it are developed in \autoref{sec:discussion}.
