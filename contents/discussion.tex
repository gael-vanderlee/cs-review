The prior review of cybersickness EEG research by Chang et al.~\cite{changBrainActivityCybersickness2023} reported delta-band increases, beta-band decreases, and inconsistent alpha-band findings; the present corpus refines this picture in two respects.
...
\subsection{Objective Markers and the Limits of Self-Report}\label{sec:objective-markers}
VIMS has no diagnostic biomarker; its ground truth is self-report.
It is defined by a symptom profile (nausea, disorientation, and oculomotor disturbance) composed of private experiences that cannot be externally verified \cite{kennedySimulatorSicknessQuestionnaire1993, stanneyCybersicknessNotSimulator1997}; any candidate objective marker must therefore be calibrated against self-report rather than replacing it.
Functional cognitive consequences of this state are documented in 4 corpus studies: working-memory decrements scaling with reported severity \cite{mimnaughVirtualRealitySickness2023}, inhibitory-control demand reflected in N2/P3 components \cite{wuInhibitionrelatedN2P32020}, and degraded sensorimotor-rhythm neurofeedback performance under cybersickness \cite{bergerInfluencePlaceboTDCS2024, bergerManipulatingCybersicknessVirtual2025}, indicating that the state extends beyond subjective discomfort and carries measurable functional costs.
...
\subsection{Methodological Challenges and Critical Perspective}\label{sec:challenges}
