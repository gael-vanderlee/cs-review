...
8. `[R3]` Comment: The Discussion and Future Directions sections are too narrative and are not grounded enough in the reviewed corpus.
   - Done: Phase 6 wholesale rewrite of `discussion.tex`, `challenges.tex`, `future.tex` (17 paragraph slots). Every Discussion claim anchored to a corpus count (16/30 delta, 20/35 theta, 25/92 sway/eye-tracker co-recording, 5/77 eyes-closed baseline, 13/38 accuracy-only, 19/38 no threshold, 8/38 numeric cutoff, 1/38 LOSO-CV, 58/72 ≤32 channels, 14/72 ≤10 channels, 43/77 no reference electrode, 59/77 no notch filter, 11/92 male-only, 12.6/23.0 min stimulus-duration confound) or a cited primary source. Chang comparison repurposed from scope-recap (already in Intro) to findings-comparison.
   - Done (infra): Egger methodology fixed in `stats_tests.py` (one effect per study); corrected p=0.036, k=27 propagated to Results §3.6 and Challenges; abstract and Intro contribution #3 reframed around reporting-gap counts instead of publication bias.
...
11. `[R1, R3]` Comment: The machine learning discussion relies too heavily on `accuracy` and should better reflect issues such as sensitivity/specificity and arbitrary sick versus non-sick thresholds.
    ...
    - Done (Results): Phase 6 corrected Egger in §3.6 to p=0.036, k=27 with small-pool / heterogeneity caveat; no longer claims publication bias.
    - Done (Contribution #3): Reframed away from the prior Egger publication-bias headline (which used an unsafe methodology — double-counted within+cross rows with n=30 imputation; corrected re-run with one effect per study yields p=0.036 / k=27 and is reported in Challenges as exploratory only, not as a contribution anchor). The methodological-audit contribution is now anchored on the reporting-gap counts above.
...
14. `[R1]` Comment: Objective measures of cybersickness should be discussed more carefully because cybersickness and motion sickness are subjective phenomena.
    - Done (Discussion): Phase 6 dedicates a Discussion subsection ("Objective Markers and the Limits of Self-Report") stating that cybersickness has no diagnostic biomarker and that ground truth is self-report. Postural sway is presented as the theoretically anchored candidate (postural-instability account; Riccio & Stoffregen 1991, Stoffregen & Smart 1998; VR-specific coupling in Palmisano 2014, Dennison & D'Zmura 2018, Teixeira 2024). Pupillometry is presented as a second candidate (Kourtesis 2023 × 2) with arousal and cognitive-load confounds named. Corpus context: 25 of 92 corpus studies already co-record force-plate, balance-board, eye-tracker, or pupillometry alongside EEG but typically as covariates rather than as candidate ground truth.
