...
1. `[AE, R1]` Comment: The manuscript framing around `BCI` is not convincing, and the title, abstract, and keywords are affected by that framing.
   ...
   - Done: Title changed to "The Neurophysiology of Cybersickness: A Systematic Review of Neuromarkers, Neurostimulation, and Reporting Practices in Virtual Reality"; abstract and introduction rewritten around "neuromarkers" as the scope-defining concept; "BCI" removed throughout.
...
4. `[AE, R2]` Comment: The manuscript is inconsistent about calling itself a systematic vs. scoping review.
   - Decision: The manuscript already calls itself "systematic review" consistently everywhere (title, abstract, intro, methodology, conclusion) and only ever calls Chang et al. 2023 a "scoping review" (accurate — that is the title of their paper). The real inconsistency was in how Chang et al.'s paper was labeled across sections ("a previous review" in the introduction vs. "the prior scoping review" in the discussion/conclusion), which could read as a second self-description on a skim. Per user direction, dropped the word "scoping" entirely and standardized all four mentions of Chang et al.'s paper to "prior review".
   - Done: `contents/introduction.tex` ("In a prior review, Chang et al..."), `contents/discussion.tex` ("The prior review of cybersickness EEG research by Chang et al..."), `contents/conclusion.tex` ("...the prior review of Chang et al."), `contents/challenges.tex` ("...the alpha-band inconsistency reported in the prior review..." — was plural "prior reviews"). Zero remaining hits for "scoping" anywhere in `contents/*.tex`. Manuscript compiles clean at 20 pages.
5. `[AE, R2]` Comment: P8 numbers are inconsistently given with or without percentages; add percentages throughout.
   - Done: Added bracketed percentages (denominator = 92-study corpus, matching the paper's existing convention) to every bare count on P8 in `contents/results.tex`: physiological co-recording modalities (line 53: EOG, ECG, GSR, EMG, EGG, PPG, respiration, skin temperature, blood pressure), the multi-method overlap count and questionnaire-instrument counts (lines 72-73: SSQ, FMSS, MSSQ, VRSQ, other, none), and baseline-condition counts (line 76: task, eyes-open, pre-stimulus, eyes-closed, unspecified, unreported). Verified all 22 added percentages against the /92 denominator with a standalone Python script; baseline counts also confirmed to sum to exactly 92.
...
7. `[R3]` Comment: P10 `any-mention` terminology is unclear.
8. `[R3]` Comment: P11-12 frequency ranges should be stated systematically for each band (only delta/theta currently given).
9. `[Editor letter]` Comment: A separate, specific response to every Associate Editor and reviewer comment must accompany the revision.
